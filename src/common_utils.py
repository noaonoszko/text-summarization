...
def create_vocab_emb_dict(word_emb_file_name, word_emb_size):
    ...
    with open(word_emb_file_name, "r", encoding='utf-8') as emb_file:
        for line_count, line in enumerate(emb_file):
            line_content = line.split()
            # print(line_content)
            if line_count == 0:
                vocab_size = int(line_content[0])
                word_emb_array = np.empty((vocab_size, word_emb_size), np.float32)
            else:
                vocab_dict[line_content[0]] = line_count + 1
                emb_instance_data = [float(emb_item) for emb_item in line_content[1:word_emb_size+1]]#[0:SummarizerParameters.word_emd_dim]
                word_emb_array[line_count - 1] = emb_instance_data

    return vocab_dict, word_emb_array
...
def train_loader(p, wordvecs, word_int_dict, train_set, batch_size=5, emb_size=50):
    ...
    for b in range(n_batches):
        ...
        actual_batch_size = batch_size if b < n_batches - 1 else len(train_set)-batch_size*b
